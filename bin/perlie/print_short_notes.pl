...
sub process_song {
    ...
    ## Stuff for finding the short notes
    my $na = $song->notearr();
    my @sust = ();
    for my $i ( 0 .. @$na-1 ) {
	next unless $na->[$i]->sustain();
	my $ct = $na->[$i]->currTempo();
	my $ms  = int (1000 * $na->[$i]->lenTick() / 480.0 * $ct->tempo() / 1.0e6);
	##my $ms2 = int (1000 * ($na->[$i]->lenTick() - 120) / 480.0 * $ct->tempo() / 1.0e6);

	push @sust, [$i,$na->[$i]->lenTick(),$na->[$i]->notestr(),$na->[$i]->startMeas(), $ms, int(60.0e6 / $ct->tempo()+0.5)];
    }

    @sust = sort { $a->[4] <=> $b->[4] or $a->[0] <=> $b->[0] } @sust;
    foreach my $sust (@sust) { printf "%4d %4d %4d %-3s %4d %7.3f\n", $sust->[1], $sust->[4], $sust->[5], $sust->[2], $sust->[0], $sust->[3]; }
}
